...
def test_mark_filled_hl_updates_all_fields():
    """mark_filled_hl sets exchange_status, hl_oid, filled_contracts, avg_fill_px."""
    order = Order(
        action=ActionType.BUY_ADDON,
        symbol="BTC",
        shares=0,
    ...
    assert updated.filled_contracts == pytest.approx(0.1)
...
def test_order_has_hl_fields_with_correct_defaults():
    ...
    assert order.filled_contracts == pytest.approx(0.0)
...
def test_build_order_still_works_without_hl_fields():
    ...
    decision = Decision(action=ActionType.BUY_BASE, shares=10, reason="test")
...
def test_mark_filled_hl_handles_none_avg_fill_px():
    ...
    assert updated.filled_contracts == pytest.approx(0.05)
