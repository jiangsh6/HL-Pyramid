...
def test_reduce_latest_addon_when_only_close_below_ma5():
    ...
    assert d.shares == 4   # latest addon (add_3) only
...
def test_base_not_double_reduced_when_ma20_and_drawdown_both_trigger():
    ...
    d = check_base_reduce(state, ind, cfg)
    assert d.shares == 11   # half once, not twice
...
def test_lifo_partial_reduce_exhausts_latest_lot_first():
    lots = [
        LotRecord(lot_id="a", entry_price=10, shares=5,
                  entry_date="2026-01-01"),
        LotRecord(lot_id="b", entry_price=20, shares=3,
                  entry_date="2026-01-02"),
        LotRecord(lot_id="c", entry_price=30, shares=4,
                  entry_date="2026-01-03"),
    ]
    updated, sold = lifo_reduce(lots, shares_to_sell=4)
    assert sold == 4
    # 'c' (latest, 4 shares) fully sold; 'a' and 'b' untouched
    assert len(updated) == 2
    assert updated[0].lot_id == "a"
    assert updated[1].lot_id == "b"
    assert updated[1].shares == 3
...
def test_lifo_reduce_splits_partially_exhausted_lot():
    ...
    updated, sold = lifo_reduce(lots, shares_to_sell=6)
    assert sold == 6
    # c (4) consumed; b reduced by 2 → 1 remaining
    assert len(updated) == 2
    assert updated[1].lot_id == "b"
    assert updated[1].shares == 1
...
def test_avg_entry_recalculated_correctly_after_reduce():
    ...
    assert state.current_position_shares == 36
    ...
    assert state.current_position_shares == 30


def test_reduce_mode_to_exited_when_base_lot_shares_reach_zero():
    cfg = base_config()
    state = make_state(
        state=BotState.REDUCE_MODE,
        base_lot=make_lot("base", 900.0, 22),
