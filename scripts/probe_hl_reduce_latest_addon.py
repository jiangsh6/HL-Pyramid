"""One-cycle, controlled Hyperliquid LIFO reduce-latest-addon probe.

This script bypasses strategy signals but not lifecycle safety:
  - testnet only
  - one reduce-only SELL for the latest addon lot only
  - no recurring loop
...
def main() -> int:
    parser = argparse.ArgumentParser(description="One-cycle controlled HL testnet reduce-latest-addon probe")
    ...
    if terminal:
        print("STOPPED_BEFORE_REDUCE: local_state_load_terminal")
        return 1
    if state.pending_order is not None:
        print("STOPPED_BEFORE_REDUCE: local_pending_order_exists")
        return 1
    if state.state not in {BotState.PYRAMID_LONG, BotState.REDUCE_MODE}:
        print("STOPPED_BEFORE_REDUCE: local_state_not_pyramid")
        print("state=" + state.state.value)
        return 1
    if state.base_lot is None or not state.addon_lots:
        print("STOPPED_BEFORE_REDUCE: missing_base_or_addon")
        return 1

    base_qty_before = state.base_lot.qty
    latest_addon = state.addon_lots[-1]
    addon_qty_before = latest_addon.qty
    if addon_qty_before <= EPSILON:
        print("STOPPED_BEFORE_REDUCE: addon_qty_zero")
        return 1
    ...
    if snapshot.open_orders_count > 0:
        print(f"STOPPED_BEFORE_REDUCE: open_orders_count={snapshot.open_orders_count}")
        return 1
    ...
    if exchange_qty <= EPSILON:
        print("STOPPED_BEFORE_REDUCE: no_exchange_position")
        return 1
    if abs(exchange_qty - state.current_position_qty) > EPSILON:
        print("STOPPED_BEFORE_REDUCE: local_exchange_qty_mismatch")
    ...
    if reconciliation.status.value == "halted" or state.halted:
        ...
        print("STOPPED_BEFORE_REDUCE: reconciliation_halted")
        print("halt_reason=" + str(state.halt_reason))
        return 1
    if state.pending_order is not None:
        print("STOPPED_BEFORE_REDUCE: pending_after_reconciliation")
        return 1
    ...
    raw_qty = addon_qty_before if args.qty is None else float(args.qty)
    ...
    if blocker is not None:
        print(f"STOPPED_BEFORE_REDUCE: {blocker} qty={decimal_to_plain_string(qty_dec)}")
        return 1
    if qty - addon_qty_before > EPSILON:
        print("STOPPED_BEFORE_REDUCE: qty_exceeds_latest_addon")
        return 1
    ...
    decision = Decision(
        ...
        qty=qty,
        reason="manual_controlled_lifo_reduce_latest_addon_probe",
        new_state=BotState.BASE_LONG,
    )
    print("pre_reduce_state=" + state_before)
    print(f"exchange_qty={exchange_qty}")
    print("reduce_side=sell")
    print("reduce_only=True")
    print("raw_reduce_qty=" + str(raw_qty))
    print("reduce_qty=" + decimal_to_plain_string(qty_dec))
    print("base_qty_before=" + str(base_qty_before))
    print("latest_addon_qty_before=" + str(addon_qty_before))
